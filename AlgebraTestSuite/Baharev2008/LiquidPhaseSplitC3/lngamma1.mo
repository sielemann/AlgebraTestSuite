...
protected
  ...
  Real s1 = (G21^2*tau21*x2^2+G21*G31*(tau21+tau31)*x2*x3+G31^2*tau31*x3^2)/(x1+G21*x2+G31*x3)^2;
  Real s2 = (G12*tau12*x2^2+G12*G32*(tau12-tau32)*x2*x3)/(G12*x1+x2+G32*x3)^2;
  Real s3 = (G13*tau13*x3^2+G13*G23*(tau13-tau23)*x2*x3)/(G13*x1+G23*x2+x3)^2;
algorithm
  lngamma := s1+s2+s3;
